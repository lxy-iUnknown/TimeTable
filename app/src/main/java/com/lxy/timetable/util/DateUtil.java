...
public class DateUtil {
    ...
    private static long weekCountInternal(long start, long end) {
        if (BuildConfig.DEBUG) {
            Contract.require(start >= 0, "Start date(" + start + ") < 0");
            Contract.require(end >= 0, "End date(" + end + ") < 0");
            Contract.require(start <= end, "Start date(" + start + ") > end date(" + end + ")");
        }
    ...
    public static long now() {
        ...
        long date = System.currentTimeMillis() / DIVIDEND;
